...
TruthFast `v1.0.0` is the curated public reference distribution. The exact
runtime-qualified engineering source was developed and qualified in the private
`computeaholic/ThreadForge` engineering repository at revision
`0ddae102badf2a93fe4fdb3934ad9a36db4c8c84`.

The public TruthFast release was derived from that qualified source and the
pre-public release verification found no semantic delta in runtime code,
runtime security policy, proof behavior, or qualification-relevant
infrastructure. The public release commit and tag are therefore not represented
as independently runtime-qualified revisions; their qualification provenance
traces to the exact engineering source identified above.

Historical ThreadForge identifiers remain where they represent genuine
provenance or runtime identity. They are not competing public system identity.
...
- **Public release provenance:** [PUBLIC_RELEASE_PROVENANCE.md](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/releases/PUBLIC_RELEASE_PROVENANCE.md)
- **Paper license:** [Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/)
- **Reference implementation license:** source-available under the [PolyForm Perimeter License 1.0.1](https://github.com/computeaholic/TruthFast/blob/v1.0.0/LICENSE); third-party components retain their respective upstream licenses and notices
- **Patent status:** Patent pending; see [PATENTS.md](https://github.com/computeaholic/TruthFast/blob/v1.0.0/PATENTS.md)

> **Patent pending.** Certain technical mechanisms described in this paper are
> the subject of a U.S. provisional patent application filed on September 1,
> 2026. No patent grant is implied by this notice.
...
The [Engineering Lexicon](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/ENGINEERING_LEXICON.md) owns the formal definitions. This paper owns the architectural relationship among them.
...
Profiles, providers, collectors, evaluators, scripts, and runtime controls are implementation. They realize constitutional objects but cannot redefine them. This direction of authority is what keeps an implementation limitation from quietly becoming a weaker institutional claim. The [Constitution](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/01-Constitution.md) and [assurance reference architecture](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/00-ThreadForge-Assurance-Reference-Architecture.md) own the formal dependency rules.
...
Autonomy does not collapse decision and execution authority. Autonomous agents are one application domain, not the constitutional unit. The reference repository's [CIV intelligence surfaces](https://github.com/computeaholic/TruthFast/blob/v1.0.0/platform/runtime/civ/README.md) produce deterministic, non-binding advisory artifacts and are mechanically prohibited from enforcement. An external authority decides whether an operation may proceed. A recommendation is input to governance, not permission to execute.

In the native implementation, SPIFFE identities issued through SPIRE identify workloads. SPIRE owns the trust authority, and the Istio certificate path is rooted in that authority. Active authority, issuance authority, and bundle membership remain distinct during legitimate rollover; the selected runtime lineage must remain coherent across SPIRE, Istio, and Envoy. Missing or inconsistent trust state fails the relevant contract rather than activating an undeclared fallback. The [identity](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/IDENTITY.md) and [trust](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/TRUST_MODEL.md) documents own the implementation mechanics.
...
Failure also remains evidence. A failed guarantee is `FAIL`; a guarantee that did not execute because a dependency failed is `NOT_EVALUATED`; a required path that could not execute is `BLOCKED`. Missing prerequisites, policy violations, contract violations, and system regressions remain distinct because each permits a different conclusion. The [Evidence Contracts](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/05-Evidence-Contracts.md), [Proof Model](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/PROOF_MODEL.md), and [Proof Contract](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/PROOF_CONTRACT.md) own the exact semantics.
...
`make prove-spire-outage` is not an ordinary proof phase: it first demonstrates that normal authority is policy-blocked, then requires an explicit audited break-glass authority before inducing a real SPIRE outage. The canonical [architecture](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/ARCHITECTURE.md), [security model](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/SECURITY_MODEL.md), [policy contract](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/POLICY_ENFORCEMENT.md), and [supply-chain model](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/CANONICAL/SUPPLY_CHAIN.md) own the exact native scope.
...
| Qualification operation | Result | Operation identity / evidence note |
| --- | --- | --- |
| Destructive Golden Boot reconstruction | PASS | `20260830T185319Z-1815075` |
| Post-reconstruction registry validation | PASS | durable configuration, CA/TLS and authenticated registry contract |
| Post-reconstruction SPIRE storage validation | PASS | canonical storage producer contract |
| Supported `demo-all`, run 1 | PASS | `supported-demo-all-20260830T202457Z`; source mutation `0`; manual runtime repairs `0` |
| Supported `demo-all`, run 2 | PASS | `supported-demo-all-20260830T202831Z`; source mutation `0`; manual runtime repairs `0` |
| Controlled SPIRE outage / fail-closed witness | PASS | bounded outage, denial and recovery behavior |
| Proof tamper witness | PASS | tampered evidence rejected; canonical proof unchanged |
| Registry audit | PASS | release-facing registry invariants satisfied |
| Canonical operator audit | PASS | `make audit` |
| Final repository/proof verification | PASS | `make verify-main` |
...
No conclusion may carry more authority than its claim, operation, evidence, proof, and governance permit. The [Certification Architecture](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/07-Certification-Architecture.md) and [Governance](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/14-Governance.md) define the model. The release evidence record owns the current exact-source runtime qualification boundary.
...
A reviewer can follow the active authority chain without reading historical convergence material. The public TruthFast repository is a curated V1 distribution whose executable content traces to the exact ThreadForge engineering source qualified at revision `0ddae102badf2a93fe4fdb3934ad9a36db4c8c84`. Public documentation links below are pinned to release tag `v1.0.0`; the public release provenance record documents the mapping between those two identities.

1. Read this paper for the architectural thesis and the exact-source qualification boundary, then inspect the [public release provenance record](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/releases/PUBLIC_RELEASE_PROVENANCE.md).
2. Read the [assurance reference architecture](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/00-ThreadForge-Assurance-Reference-Architecture.md), [Constitution](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/01-Constitution.md), and [Concept Index](https://github.com/computeaholic/TruthFast/blob/v1.0.0/docs/architecture/17-Concept-Index.md) for ownership and dependency rules.
3. Read the relevant claim, evidence, proof, identity, trust, policy, and certification contracts under [`docs/architecture/`](https://github.com/computeaholic/TruthFast/tree/v1.0.0/docs/architecture/) and [`docs/CANONICAL/`](https://github.com/computeaholic/TruthFast/tree/v1.0.0/docs/CANONICAL/).
4. Inspect the native execution entry points (`make validate-all`, `make golden-boot`, `make demo-all`, `make audit`, `make forgesec`) in the TruthFast `v1.0.0` distribution. Their qualification provenance traces to engineering source revision `0ddae102badf2a93fe4fdb3934ad9a36db4c8c84` and the evidence package bound to that revision.
5. Reproduce or challenge the system independently rather than treating this paper or the originating platform's `PASS` as third-party validation.
