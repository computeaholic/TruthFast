...
# Runner: executes the canonical civ_cpu_governance_test.sh and produces artifacts.

set -euo pipefail

TS=$(date -u +%Y%m%dT%H%M%SZ)
ART=artifacts/civ/cpu-governance-test/${TS}
mkdir -p "${ART}"

TMP_OUT=$(mktemp -t civ_cpu_governance.XXXXXX)
EXIT=0
if tools/verify/civ/civ_cpu_governance_test.sh >"${TMP_OUT}" 2>&1 ; then
  EXIT=0
else
  EXIT=$?
fi
...
- Vector databases, if used for analysis elsewhere, are treated as semantic lenses only and are not authoritative.
